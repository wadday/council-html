...
const Group = {
    template: `
    <div class="max-w-xl mx-auto space-y-5 rounded-lg border shadow-lg border-gray-100 border-opacity-40 bg-white bg-opacity-50 backdrop-blur p-3">
     <div class="p-4 bg-gray-100 rounded-lg shadow-lg border border-opacity-40">
      <h1 class="mb-3">Age Groups</h1>
         <form class="space-y-6" @submit.prevent="save">
              <div>
                ...
                <div>
    <label for="name" class="block text-sm font-medium text-gray-700">ބަލައިގެން</label>
                  <div class="mt-1 border-b border-gray-300 focus-within:border-indigo-600">
                    <input  v-model="form.name"
                            type="text"
                            name="name"
            ...
            <div class="flex justify-between space-x-3">
              <div class="min-w-0 flex-1">
                <div class="block focus:outline-none">
                  <router-link :to="{ name: 'config-options', params: { id: item.id }}" class="truncate text-sm font-medium text-gray-900">{{ item.name }}</router-link>
                </div>
              </div>
